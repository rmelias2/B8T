...
ggsave(filename = "CD8TGCS_ABACUS_Pre_Post.pdf", path = output.path, plot = g, device = "pdf", width = 2, height = 3)
...
ggsave(filename = "BCGS_ABACUS_Pre_Post.pdf", path = output.path, plot = g, device = "pdf", width = 2, height = 3)
...
plot <- plot %>% dplyr::filter(!ptID %in% no_follow_up)


nodes <- data.frame(name = c("Pre Hi/Hi", "Pre Hi/Lo", "Pre Lo/Hi", "Pre Lo/Lo", "Post Hi/Hi", "Post Hi/Lo", "Post Lo/Hi", "Post Lo/Lo"))

reshaped_data <- plot %>%
  pivot_wider(names_from = VISIT, values_from = B8T) %>% dplyr::rename(PRE = `PRE`, POST = `POST`)
